
## Completed
- [x] Analyzed codebase (App.jsx, SendView, TransferList, usePeerConnections, useSocket, server, CSS)
- [x] Confirmed plan with user

## New Features
- [x] Desktop Notifications (requests + completion)
- [x] Global Drag & Drop to send
- [x] Sound alerts (toggleable)
- [x] Auto-accept from favorite devices
- [x] Image preview before/after receiving
- [x] Chat interface for text messages
- [x] History export (CSV/JSON)
- [x] Settings toggles integration
- [x] Send page redesign (full page, not modal)
- [x] Build client + verify
- [x] Unread chat badge + real-time chat update
- [x] Commit & push to GitHub
